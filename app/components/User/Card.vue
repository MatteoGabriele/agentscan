...
<template>
  ...
  >
    ...
    <div class="w-full flex flex-col">
      ...
      <ul class="text-gh-muted @md:mt-4 text-sm hidden @xl:flex flex-wrap gap-4 flex-row">
        <li class="flex items-center gap-1">
          <span class="i-lucide:users hidden @md:flex shrink-0" aria-hidden="true" />
          {{ user.followers }} followers
        </li>
        <li class="flex items-center gap-1">
          <span class="i-lucide:package hidden @md:flex shrink-0" aria-hidden="true" />
          <span v-if="user.public_repos === 0">No repos</span>
          <span v-else>{{ user.public_repos }} repos</span>
        </li>
        <li class="flex items-center gap-1">
          <span class="i-lucide:calendar hidden @md:flex shrink-0" aria-hidden="true" />
          Since
