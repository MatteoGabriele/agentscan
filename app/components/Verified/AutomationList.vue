    ...
    <div class="flex flex-wrap items-center justify-center gap-2 min-h-[30px]">
      ...
      <template v-else>
        <NuxtLink
          ...
          class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-full border border-ui-border/40 bg-white/2 hover:bg-white/4 hover:border-ui-border/60 transition-all"
          @click="trackEvent('automation_list_account_link_clicked')"
        >
          <span class="text-ui-text">@{{ account.username }}</span>
        </NuxtLink>

        <NuxtLink
          :to="{ name: 'automations' }"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-full border border-ui-border/20 bg-white/1 text-ui-muted hover:bg-white/2 hover:border-ui-border/40 hover:text-ui-text transition-all"
        >
