...
export function reviewProcessComment(thresholds: Thresholds): string {
  ...
  return [
    ...
    ``,
    `**No single maintainer decides this.** The issue has been announced to the review team, and each reviewer votes on it by reacting to the **issue itself**:`,
    ``,
    `- 👍 the evidence shows an automated account`,
    `- 👎 the evidence is not conclusive`,
    ``,
    `A bot counts those reactions every hour and closes this issue once the outcome is settled:`,
    ``,
    ...
    `- A report is also rejected as soon as enough 👎 make ${minApprovals} 👍 impossible.`,
    ``,
    `**Only reactions from the review team are counted.** Reactions from anyone else — including yours, and including anyone you ask to come and react — are ignored, so they cannot move a report either way.`,
    ``,
    `There is nothing further you need to do. If you come across stronger evidence in the meantime, add it as a comment: reviewers can change their reaction at any point before the report is settled.`,
    ``,
    `A rejection is not a judgement on you or on the reported account — it only means the reviewers did not find the evidence conclusive.`,
  ].join('\n')
}
