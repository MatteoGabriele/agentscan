...
export default defineEventHandler(async () => {
  try {
    const raw = await useStorage('assets:data').getItemRaw(
      'scan-results-tmp.txt',
    )

    if (!raw) {
      throw new Error('scan-results-tmp.txt not found')
    }
    ...
    console.error('Ecosystem health (current week) fetch error:', error)
    throw createError({
      statusCode: 500,
      message: 'Failed to fetch current week health data',
    })
  }
})
